...
impl TranslateRiscv {
    pub fn translate_jal(&mut self, inst: &InstrInfo) -> Vec<TCGOp> {
        ...
        if rd_addr != 0 {
            tcg_lists.push(TCGOp::new_2op(TCGOpcode::MOV_IMM_64BIT, Rc::clone(&dest_temp), next_pc));
            tcg_lists.push(TCGOp::tcg_set_gpr(rd_addr, Rc::clone(&dest_temp)));
        }
        tcg_lists.push(TCGOp::new_2op(TCGOpcode::JMPIM, Rc::clone(&dest_temp), imm));
    ...
    pub fn translate_jalr(&mut self, inst: &InstrInfo) -> Vec<TCGOp> {
        ...
        if rd_addr != 0 {
            ...
            tcg_lists.push(TCGOp::tcg_set_gpr(rd_addr, Rc::clone(&dest)));
        }
        tcg_lists.push(TCGOp::new_3op(TCGOpcode::JMPR, Rc::clone(&dest), Rc::clone(&source1), imm));
        tcg_lists.push(TCGOp::new_0op(TCGOpcode::EXIT_TB, None));
    ...
    pub fn translate_lui(&mut self, inst: &InstrInfo) -> Vec<TCGOp> {
        ...
        let source1 = self.tcg_temp_new();
        let rs1_op = TCGOp::tcg_get_gpr(Rc::clone(&source1), 0);  // Box::new(TCGv::new_reg(rs1_addr as u64));
        let tcg_inst = TCGOp::new_3op(TCGOpcode::ADD_64BIT, Rc::clone(&source1), Rc::clone(&source1), tcg_imm);
        let rd_op = TCGOp::tcg_set_gpr(rd_addr, Rc::clone(&source1));  // Box::new(TCGv::new_reg(rs1_addr as u64));
        self.tcg_temp_free(Rc::clone(&source1));
    ...
    pub fn translate_auipc(&mut self, inst: &InstrInfo) -> Vec<TCGOp> {
        ...
        if rd_addr != 0 {
            tcg_lists.push(TCGOp::new_2op(TCGOpcode::MOV_IMM_64BIT, Rc::clone(&dest_temp), imm));
            tcg_lists.push(TCGOp::tcg_set_gpr(rd_addr, Rc::clone(&dest_temp)));
        }
        self.tcg_temp_free(Rc::clone(&dest_temp));
    ...
    pub fn translate_addiw(&mut self, inst: &InstrInfo) -> Vec<TCGOp> {
        ...
        let source1 = self.tcg_temp_new();
        let rs1_op = TCGOp::tcg_get_gpr(Rc::clone(&source1), rs1_addr);  // Box::new(TCGv::new_reg(rs1_addr as u64));
        let tcg_inst = TCGOp::new_3op(TCGOpcode::ADD_32BIT, Rc::clone(&source1), Rc::clone(&source1), tcg_imm);
        let tcg_sign_ext = TCGOp::new_2op(TCGOpcode::SIGN_EXT_32_64, Rc::clone(&source1), Rc::clone(&source1));
        let rd_op = TCGOp::tcg_set_gpr(rd_addr, Rc::clone(&source1));  // Box::new(TCGv::new_reg(rs1_addr as u64));
        self.tcg_temp_free(Rc::clone(&source1));
    ...
    pub fn translate_raw_load(&mut self, base_reg: u32, offset: u64, dest_reg: u32, inst: &InstrInfo, load_op: TCGOpcode, helper_op: CALL_HELPER_IDX) -> Vec<TCGOp> {
        ...
        // Read Register
        tcg_lists.push(TCGOp::tcg_get_gpr(Rc::clone(&src_addr), base_reg));
        // Extract TLB Index and offset
        if offset != 0 {
            tcg_lists.push(TCGOp::new_3op(TCGOpcode::ADD_64BIT, Rc::clone(&src_addr), Rc::clone(&src_addr), TCGv::new_imm(offset as u64)));
        }
        tcg_lists.push(TCGOp::new_3op(TCGOpcode::AND_64BIT, Rc::clone(&vaddr_low12bit), Rc::clone(&src_addr), TCGv::new_imm(0xfff)));

        tcg_lists.push(TCGOp::new_3op(TCGOpcode::SRL_64BIT, Rc::clone(&vaddr_tlb_idx), Rc::clone(&src_addr), TCGv::new_imm(12)));
        tcg_lists.push(TCGOp::new_3op(TCGOpcode::AND_64BIT, Rc::clone(&vaddr_tlb_idx), Rc::clone(&vaddr_tlb_idx), TCGv::new_imm(0xfff)));
        tcg_lists.push(TCGOp::new_3op(TCGOpcode::SLL_64BIT, Rc::clone(&vaddr_tlb_idx), Rc::clone(&vaddr_tlb_idx), TCGv::new_imm(3)));

        // Make TLB Vaddr Index Address
        tcg_lists.push(TCGOp::new_1op(TCGOpcode::MOVE_STACK, Rc::clone(&stack_reg)));
        tcg_lists.push(TCGOp::new_2op(TCGOpcode::ADD_TLBIDX_OFFSET, Rc::clone(&tlb_byte_addr), Rc::clone(&stack_reg)));  // Relative Addr of TLB
        tcg_lists.push(TCGOp::new_3op(TCGOpcode::ADD_64BIT, Rc::clone(&tlb_byte_addr), Rc::clone(&tlb_byte_addr), Rc::clone(&vaddr_tlb_idx)));

        // Make VAddr upper bit for compare TLB value
        tcg_lists.push(TCGOp::new_3op(TCGOpcode::SRL_64BIT, Rc::clone(&src_addr), Rc::clone(&src_addr), TCGv::new_imm(24)));
        tcg_lists.push(TCGOp::new_2op(TCGOpcode::MEM_LOAD, Rc::clone(&tlb_byte_addr), Rc::clone(&tlb_byte_addr)));
        tcg_lists.push(TCGOp::new_2op_with_label(TCGOpcode::CMP_EQ, Rc::clone(&src_addr), Rc::clone(&tlb_byte_addr), Rc::clone(&label_tlb_match)));
        ...
        tcg_lists.push(TCGOp::new_4op(TCGOpcode::EQ_EAX_64BIT, Rc::clone(&src_addr), Rc::clone(&zero),  dummy_addr, Rc::clone(&label_load_excp)));
        tcg_lists.push(TCGOp::new_0op(TCGOpcode::EXIT_TB, None));

        // Extract lower 12bit address and add with TLB address
        tcg_lists.push(tcg_label_tlb_match);
        tcg_lists.push(TCGOp::new_1op(TCGOpcode::MOVE_STACK, Rc::clone(&stack_reg)));
        tcg_lists.push(TCGOp::new_2op(TCGOpcode::ADD_TLBADDR_OFFSET, Rc::clone(&tlb_byte_addr), Rc::clone(&stack_reg)));  // Relative Addr of TLB Paddr
        tcg_lists.push(TCGOp::new_3op(TCGOpcode::ADD_64BIT, Rc::clone(&tlb_byte_addr), Rc::clone(&tlb_byte_addr), Rc::clone(&vaddr_tlb_idx)));
        tcg_lists.push(TCGOp::new_2op(TCGOpcode::MEM_LOAD, Rc::clone(&tlb_byte_addr), Rc::clone(&tlb_byte_addr)));
        tcg_lists.push(TCGOp::new_3op(TCGOpcode::ADD_64BIT, Rc::clone(&tlb_byte_addr), Rc::clone(&tlb_byte_addr), Rc::clone(&vaddr_low12bit)));
        tcg_lists.push(TCGOp::new_3op(TCGOpcode::ADD_64BIT, Rc::clone(&tlb_byte_addr), Rc::clone(&tlb_byte_addr), TCGv::new_imm(0x80000000)));
        tcg_lists.push(TCGOp::new_2op(TCGOpcode::ADD_MEM_OFFSET, Rc::clone(&tlb_byte_addr), Rc::clone(&tlb_byte_addr)));
        tcg_lists.push(TCGOp::new_2op(load_op, Rc::clone(&tlb_byte_addr), Rc::clone(&tlb_byte_addr)));
        tcg_lists.push(TCGOp::tcg_set_gpr(dest_reg, Rc::clone(&tlb_byte_addr)));
        tcg_lists.push(tcg_label_load_excp);

        self.tcg_temp_free(Rc::clone(&src_addr      ));
        self.tcg_temp_free(Rc::clone(&vaddr_low12bit));
        self.tcg_temp_free(Rc::clone(&vaddr_tlb_idx ));
        self.tcg_temp_free(Rc::clone(&stack_reg     ));
        self.tcg_temp_free(Rc::clone(&tlb_byte_addr ));
...
// 
        ...
        tcg_lists.push(TCGOp::tcg_get_gpr(Rc::clone(&rs2_data), dest_reg));
        tcg_lists.push(TCGOp::new_2op(store_op, Rc::clone(&rs2_data), Rc::clone(&tlb_byte_addr)));
        tcg_lists.push(tcg_label_load_excp);

        self.tcg_temp_free(Rc::clone(&vaddr_low12bit));
        self.tcg_temp_free(Rc::clone(&vaddr_tlb_idx ));
        self.tcg_temp_free(Rc::clone(&tlb_byte_addr ));
        self.tcg_temp_free(Rc::clone(&rs2_data      ));
    ...
    fn translate_div_common (&mut self, op: TCGOpcode, rd_addr: u32, rs1_addr: u32, rs2_addr: u32, inst: &InstrInfo) -> Vec<TCGOp> {
        ...
        tcg_lists.push(TCGOp::tcg_get_gpr(Rc::clone(&source1), rs1_addr)); 
        tcg_lists.push(TCGOp::tcg_get_gpr(Rc::clone(&source2), rs2_addr));

        // when source2 == Rc::clone(&zero),  doesn't execute.
        let zero = self.tcg_temp_new();
        tcg_lists.push(TCGOp::new_2op(TCGOpcode::MOV_IMM_64BIT, Rc::clone(&zero),  TCGv::new_imm(0)));
        tcg_lists.push(TCGOp::new_4op(TCGOpcode::CMP_EQ, Rc::clone(&source2), Rc::clone(&zero),  TCGv::new_imm(0), Rc::clone(&label_src2_zero_fail)));
        self.tcg_temp_free(zero);

        // when source1 == 8000_0000_0000_0000, doesn't execute.
        let minus1 = self.tcg_temp_new();
        tcg_lists.push(TCGOp::new_2op(TCGOpcode::MOV_IMM_64BIT, Rc::clone(&minus1), TCGv::new_imm(0x8000_0000_0000_0000)));
        tcg_lists.push(TCGOp::new_4op(TCGOpcode::CMP_EQ, Rc::clone(&source1), Rc::clone(&minus1), TCGv::new_imm(0), Rc::clone(&label_cond1_fail)));
        self.tcg_temp_free(Rc::clone(&minus1));

        tcg_lists.push(TCGOp::new_3op(op, Rc::clone(&source1), Rc::clone(&source1), Rc::clone(&source2)));
        tcg_lists.push(TCGOp::tcg_set_gpr(rd_addr, Rc::clone(&source1))); 
        // Actually this is no conditional jump
        tcg_lists.push(TCGOp::new_4op(TCGOpcode::CMP_EQ, Rc::clone(&source1), Rc::clone(&source1), TCGv::new_imm(0), Rc::clone(&label_finish)));

        let zero = self.tcg_temp_new();
        tcg_lists.push(TCGOp::new_label(Rc::clone(&label_cond1_fail)));
        tcg_lists.push(TCGOp::new_2op(TCGOpcode::MOV_IMM_64BIT, Rc::clone(&zero),  TCGv::new_imm((0 - 1) as u64)));
        tcg_lists.push(TCGOp::new_4op(TCGOpcode::CMP_EQ, Rc::clone(&source2), Rc::clone(&zero),  TCGv::new_imm(0), Rc::clone(&label_cond2_fail)));
        self.tcg_temp_free(zero);
        ...
        tcg_lists.push(TCGOp::new_2op(TCGOpcode::MOV_IMM_64BIT, Rc::clone(&minus1), TCGv::new_imm((0 - 1) as u64)));
        tcg_lists.push(TCGOp::tcg_set_gpr(rd_addr, Rc::clone(&minus1))); 
        self.tcg_temp_free(Rc::clone(&minus1));
